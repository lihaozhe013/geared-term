...
| SES sessions and profiles     | partial            | versioned profile schema, profile list/save/delete IPC, `ProfileEditor.tsx` for local/WSL/SSH targets, one-shot SSH dialog, and saved SSH resolution                          | WSL confirmation, import mapping, and complete group semantics                     |
...
| SFTP                          | partial            | `SftpService`, independent SSH SFTP channel, validated list IPC, canonical listing paths, symlink-safe removal, and remote-file panel                                         | local pane, transfers/cancellation, sync behavior, and end-to-end coverage         |
...
| CMD command actions           | partial            | bounded conservative parser, content revisions, main-side revision/run validation, destructive-risk classification, command cards, and copy/insert/run gating                 | split-command presentation, stale-session UX, and E2E tests                        |
